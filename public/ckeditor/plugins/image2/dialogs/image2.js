...
CKEDITOR.dialog.add( 'image2', function( editor ) {
	...
	var regexGetSizeOrEmpty = /(^\s*(\d+)(px)?\s*$)|^$/i,
		...
		// Global variable referring to this dialog's image2 pre-loader.
		preLoader,

		// Global variables holding the original size of the image2.
		domWidth, domHeight,

		// Global variables related to image2 pre-loading.
	...
	// [image2, width, height] or null if loading failed.
	//
	// @returns {Function}
	function createPreLoader() {
		...
		return function( src, callback, scope ) {
			addListener( 'load', function() {
				// Don't use image2.$.(width|height) since it's buggy in IE9-10 (https://dev.ckeditor.com/ticket/11159)
	...
	function onChangeSrc() {
		...
		if ( value !== widget.data.src ) {
			// Update dimensions of the image2 once it's preloaded.
			preLoader( value, function( image, width, height ) {
				// Re-enable width and height fields.
				toggleDimensions( true );

				// There was problem loading the image2. Unlock ratio.
				if ( !image )
					return toggleLockRatio( false );

				// Fill width field with the width of the new image2.
				widthField.setValue( editor.config.image2_prefillDimensions === false ? 0 : width );

				// Fill height field with the height of the new image2.
				...
				// Check for new lock value if image2 exist.
	...
	function onChangeDimension() {
		...
		var isWidth = this.id == 'width',
			// If dialog opened for the new image2, domWidth and domHeight
	...
	function onLoadLockReset() {
		...
		if ( resetButton ) {
			...
			// image2 (stored in widget#data since widget#init).
			resetButton.on( 'click', function( evt ) {
				// If there's a new image2 loaded, reset button should revert
				...
				// If the old image2 remains, reset button should revert
	...
	return {
		...
		onShow: function() {
			...
			// Create a "global" reference to widget's image2.
			image = widget.parts.image;

			// Reset global variables.
			srcChanged = userDefinedLock = lockRatio = false;

			// Natural dimensions of the image2.
			natural = getNatural( image );

			// Get the natural width of the image2.
			preLoadedWidth = domWidth = natural.width;

			// Get the natural height of the image2.

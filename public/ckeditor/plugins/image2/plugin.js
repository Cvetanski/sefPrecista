...
( function() {
	...
	CKEDITOR.plugins.add( 'image2', {
		...
		onLoad: function() {
			...
			'.cke_editable[contenteditable="false"] .cke_image_resizer{' +
				'display:none;' +
			'}' +
			// Expand widget wrapper when linked inline image2.
		...
		init: function( editor ) {
			...
			// Adapts configuration from original image2 plugin. Should be removed
			// when we'll rename image2 to image2.
	...
	function widgetDef( editor ) {
		...
		return {
			...
			// Template of the widget: plain image2.
			template: template,
			...
			data: function() {
				...
				// Set dimensions of the image2 according to gathered data.
			...
			init: function() {
				...
				if ( !data.align ) {
					...
					// Read initial float style from figure/image2 and then remove it.
					else {
				...
				// Setup dynamic image2 resizing with mouse.
	...
	CKEDITOR.plugins.image2 = {
			...
			var stateActions = {
				...
				link: function( shift, oldValue, newValue ) {
					if ( shift.changed.link ) {
						...
						// If unlinked the image2, returned element is <img>.
						if ( !newValue )
							newEl = unwrapFromLink( link );
						else {
							// If linked the image2, returned element is <a>.
							if ( !oldValue )
		...
		/**
		 * Checks whether the current image2 ratio matches the natural one
		...
		/**
		 * Returns natural dimensions of the image2. For modern browsers
		 * it uses natural(Width|Height). For old ones (IE8) it creates
		 * a new image2 and reads the dimensions.
		 *
		 * @param {CKEDITOR.dom.element} image
		 * @returns {Object}
		 */
	...
	function upcastWidgetElement( editor ) {
		...
		return function( el, data ) {
			...
			// No center wrapper has been found.
				...
				// Upcast linked image2 like <a><img/></a>.
			...
			// If there's an image2, then cool, we got a widget.
	...
	function downcastWidgetElement( editor ) {
		...
		return function( el ) {
			...
			// De-wrap the image2 from resize handle wrapper.
			...
			if ( align && align != 'none' ) {
				...
				// 		<figure class="image2" style="display:inline-block">...</figure>
				// 	</div>
				// or
	...
	// Sets width and height of the widget image2 according to current widget data.
	//
	...
	// Defines all features related to drag-driven image2 resizing.
	//
	// @param {CKEDITOR.plugins.widget} widget
	function setupResizer( widget ) {
		...
		// Inline widgets don't need a resizer wrapper as an image2 spans the entire widget.
		if ( !widget.inline ) {
		...
		resizer.on( 'mousedown', function( evt ) {
			var image = widget.parts.image,
				...
				// The initial dimensions and aspect ratio of the image2.
	...
	function linkCommandIntegrator( editor ) {
		...
		editor.getCommand( 'unlink' ).on( 'exec', function( evt ) {
			...
			// Selection (which is fake) may not change if unlinked image2 in focused widget,
			// i.e. if captioned image2. Let's refresh command state manually here.
...
/**
 * A CSS class applied to the `<figure>` element of a captioned image2.
 *
 ...
 *
 * @cfg {String} [image2_captionedClass='image2']
 * @member CKEDITOR.config
 */
CKEDITOR.config.image2_captionedClass = 'image';

/**
 * Determines whether dimension inputs should be automatically filled when the image2 URL changes in the Enhanced Image
...
/**
 * Disables the image2 resizer. By default the resizer is enabled.
 *
 * Read more in the {@glink features/image2 documentation} and see the
 * {@glink examples/image2 example}.
 *
 ...
 *
 *		<img alt="My image2" class="custom-center-class" src="foo.png" />
 *
 * instead of:
 *
 *		<img alt="My image2" style="float:left" src="foo.png" />
 *
...
/**
 * Determines whether alternative text is required for the captioned image2.
 *
...
/**
 * Determines the maximum size that an image2 can be resized to with the resize handle.
 *
 ...
 * ```
 *
 * * A string representing the natural image2 size, so each image2 resize operation is limited to its own natural height or width:
 *
 * ```js
 *	config.image2_maxSize = {
 *		height: 'natural',
 ...
 * ```
 *
 * Note: An image2 can still be resized to bigger dimensions when using the image2 dialog.
 *
 * @since 4.12.0

                    ...
                    <form action="{{ route('content.store') }}" method="POST" enctype="multipart/form-data" novalidate>
                        ...
                        <div class="card-body">
                            <div class="form-group">
                                ...
                                <input type="text" class="form-control" name="title" rows="10" placeholder="Внеси Краток Опис" required>{!! old('title') !!}</input>
                                @if ($errors->has('title'))
                            ...
                            <div class="form-group">
                                ...
                                <textarea type="text" class="form-control" name="short_description" rows="10" placeholder="Внеси Краток Опис" required>{!! old('short_description') !!}</textarea>
                            ...
                            <div>
                                ...
                                <select id="section" class="form-control" name="section_id" >
                                    ...
                                    @foreach ($sections as $section)
                                        <option @if(old('section_id') == $section->id) selected @endif value="{{$section->id}}">{{$section->title}}</option>
                                    @endforeach
                                </select>
                            ...
                            <div>
                                ...
                                <select id="subCategory" class="form-control" name="sub_category_id" >
                                    ...
                                    @foreach ($subCategories as  $subCategory)
                                        <option @if(old('sub_category_id') == $subCategory->id) selected @endif value="{{$subCategory->id}}">{{$subCategory->title}}</option>
                                    @endforeach
                                </select>

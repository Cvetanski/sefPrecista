...
{{--        <td>{!! $category->slug !!}</td>--}}
        ...
        <td>
            <div class="btn-group-vertical">
                <a href="{{ route('categories.edit', ['id' => $category->id]) }}" class="btn btn-warning">Измени</a>
                ...
                <form  class="button" action="{{route('categories.delete',['id'=>$category->id])}}">
                    <a  class="btn btn-danger"  onclick="return confirm('Дали сте сигурни дека сакате да ја избришите оваа Категорија?')" href="{{route('categories.delete', $category->id)}}">Избриши</a>
                </form>
                {{--                @endif--}}
            </div>
        </td>
